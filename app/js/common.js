...
$(function() {
  ...
  // Menu navigation
  $('.hamburger, .nav__link, .nav-mob__link, .nav-mob__footer-link').click(
    function() {
  ...
  $('.test__btn').on('click', function() {
    new TimelineMax()
      ...
      .to(
        '.test__link',
        0.6,
        { ease: Power1.easeOut, opacity: 1, y: '0%', 'pointer-events': 'auto' },
        '-=0.4'
      );
  });
  ...
  var onLeftLeafClick = function() {
    var answer =
      'Видите, как легко ошибиться. Соберитесь, дальше вопросы будут сложнее.';
    ...
    new TimelineMax()
      ...
      .to(
        '.test__link',
        0.4,
        { opacity: 1, y: '0%', 'pointer-events': 'auto' },
        '-=0.4'
      );
  ...
  window.addEventListener('resize', function() {
    if (
      !$('.expert').hasClass('is-opened') &&
      !$('.expert').hasClass('is-closed')
    ) {
      setArticlesHeight();
      setImgTop();
      setExpertHeight();
  ...
  $('.expert').on('click', function() {
    ...
    var switching =
      $('.expert').hasClass('is-opened') && $(this).hasClass('is-closed');
    if (switching) {
      tl.to('.expert.is-opened .expert-closed__title', 0, { opacity: 0 })
    ...
    // Раскрытие карточек
    tl.to('.expert__inner, .expert__img', 0.3, {
      ease: easing,
      opacity: 0,
      y: 100
    })
      .to('.expert', 0.4, { ease: easing, height: '100vh' }, '+=0.2')
      .to(
        '.expert.is-closed',
        0.4,
      ...
      .from(
        $(openedContent).find('.expert-opened__title'),
        0.6,
        { ease: easing, opacity: 0, y: '-50%' },
        '-=0.4'
      )
      .from(
        $(openedContent).find('.expert-opened__article'),
        0.6,
        { ease: easing, opacity: 0, y: '100%' },
        '-=0.6'
      )
      .to(
        $(openedContent).find('.expert-opened__img'),
        0.4,
        { ease: easing, x: '0%' },
        '-=0.4'
      )
      .to(
        '.expert.is-closed .expert-closed__title',
        0,
        { 'pointer-events': 'auto' },
        '-=0.4'
      )
      ...
      .fromTo(
        $(openedContent).find('.expert-opened__inner'),
        0.6,
        { ease: easing, y: '70%', opacity: 0 },
        { ease: easing, y: '0%', opacity: 1 },
        '-=1'
      );
    ...
    var onOpenedClick = function(e) {
      ...
      var showImage = function() {
        ...
        if (opacityIndex != -1) {
          $('.expert__img').attr(
            'style',
            (imgStyle =
              imgStyle.slice(0, opacityIndex) +
              imgStyle.slice(opacityIndex + 10))
          );
        }
      };
      ...
      tl.to('.expert-opened', 0.4, {
        ...
        .to(
          '.expert',
          0.4,
          { width: '25%', height: finalHeight, onComplete: showImage },
          '-=0.4'
        )
        .fromTo(
          '.expert__inner',
  ...
  $('.expert-mob').on('click', function() {
    ...
    // Раскрытие
    tl.to($(this).find('.expert-mob__img, .expert-mob__inner'), 0.2, {
      ease: easing,
      opacity: 0,
      scale: 0
    });
    $(this)
      .find('.expert-mob__inner, .expert-mob__img')
      .slideUp(400);
    $(this)
      .find('.expert-mob-opened')
      .slideDown(400);
    ...
    var onMobCloseClick = function(e) {
      ...
      $(self)
        ...
        .slideUp(400);
      $(self)
        .find('.expert-mob__img, .expert-mob__inner')
        .slideDown(400);
      setTimeout(function() {
        tl.to($(self).find('.expert-mob__img, .expert-mob__inner'), 0.4, {
          ease: easing,
          opacity: 1,
          scale: 1
        });
      ...
      // Снимаем обработчик
      $(this)
        .find('.expert-mob-opened__close')
        .off('click', onMobCloseClick);
    };

    // Вешаем обработчик
    $(this)
      .find('.expert-mob-opened__close')
      .on('click', onMobCloseClick);
  });
  ...
  if (!isMobile) {
    var mainTextTween = new TimelineMax().to(
      '.main',
      2,
      { ease: easing, opacity: 0 },
      'l'
  ...
  if (!isMobile) {
    ...
    switch (viewportWidth) {
      case 900:
        logoVideoTween = new TimelineMax().to(
          '.logo-video',
          2,
          { ease: easing, x: '-20%', onStart: removeAnim },
          'l'
        );
        break;
      case 767:
        logoVideoTween = new TimelineMax().to(
          '.logo-video',
          2,
          { ease: easing, x: '-50%', y: '-85%', onStart: removeAnim },
          'l'
        );
        break;
      case 480:
        logoVideoTween = new TimelineMax().to(
          '.logo-video',
          2,
          { ease: easing, x: '-50%', y: '-100%', onStart: removeAnim },
          'l'
        );
        break;
      default:
        logoVideoTween = new TimelineMax().to(
          '.logo-video',
          3,
          { ease: easing, x: '-5%', onStart: removeAnim },
          'l'
        );
        break;
    }
  ...
  if (!isMobile) {
    ...
    $(descriptions).each(function(i, desc) {
      ...
      new ScrollMagic.Scene({
        ...
        .on('enter', function() {
          new TimelineMax().to(
            parentSection.find('.section-description'),
            0.9,
            { ease: Power1.easeInOut, y: '0%', opacity: 1 },
            '+=0.3'
          );
        })
        .addTo(controller);
  ...
  var leafLeftTween = new TimelineMax().to('.leaf-left', 2, {
    ease: easing,
    x: '0%',
    onComplete: animTestHead
  });

  new ScrollMagic.Scene({
    triggerElement: '#test',
    offset: -Math.floor(window.innerHeight / 3.2),
    duration: Math.floor(window.innerHeight / 2.5)
  })
    .setTween(leafLeftTween)
    .addTo(controller);

  var leafRightTween = new TimelineMax().to('.leaf-right', 2, {
    ease: easing,
    x: '0%'
  });
  ...
  var petalsTween = new TimelineMax().to('.petals .petal', 0.4, {
    ease: easing,
    opacity: 1,
    x: '0%',
    y: '0%'
  ...
  var subLeftTween = new TimelineMax().to('.subscribe__leaf-left', 1, {
    ease: easing,
    rotation: -leafRotate
  });
  ...
  new ScrollMagic.Scene({
    ...
    duration: Math.floor(window.innerHeight / 1.6)
  })
    .setTween(subLeftTween)
    .addTo(controller);

  var subRightTween = new TimelineMax().to('.subscribe__leaf-right', 1, {
    ease: easing,
    rotation: leafRotate
  });
  ...
  var prodPetalsTween = new TimelineMax()
    ...
    .to(
      '.product-petal-4',
      2,
      { ease: Power2.easeOut, x: '0%', y: '0%', scale: 1 },
      '-=2.4'
    )
    .to(
      '.product-petal-6',
    ...
    .to(
      '.product-petal-7',
      2.4,
      { ease: Power2.easeOut, x: '0%', y: '0%', scaleX: -1, scaleY: 1 },
      '-=2.3'
    )
    .to(
      '.product-petal-8',
...
var initMap = function() {
  ...
  $('.map-button').on('click', function() {
    ...
    if (btnIndex === 0) {
      ...
      map.setZoom(14);
      map.setCenter({
        lat: markers[btnIndex].getPosition().lat(),
        lng: markers[btnIndex].getPosition().lng()
      });
    }
  });
};
